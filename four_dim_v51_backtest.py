...
def _calc_signal_quality(hist, direction, entry_price):
    ...
    try:
        ...
        last = recent.iloc[-1]
        body_pct = (
            abs(float(last["close"]) - float(last["open"])) / float(last["open"]) * 100
...
def walk_forward_backtest_v51(symbol, cfg=None, min_bars=60, tail=None):
    ...
    while i < n - 1:
        ...
        for j in range(i + 1, n):
            ...
            # 硬止损 (3σ)
            sigma_stop = (
                entry_price - SIGMA_STOP_MULT * effective_atr
                if dir_T > 0
                else entry_price + SIGMA_STOP_MULT * effective_atr
            )

            if dir_T > 0:
                if lo <= current_stop:
                    exit_price, reason = current_stop, f"止损({tier_level})"
                    break
                if lo <= sigma_stop:
                    exit_price, reason = sigma_stop, "硬止损"
                    break
                    tiered_stops["hard"] += 1
                if hi >= ep["t2"]:
                    if ep["tail_enabled"]:
    ...
    return {
        ...
        "roll_skipped": roll_skipped,
        "v51_stats": {"quality_filtered": quality_filtered, "tf_filtered": tf_filtered, "tiered_stops": tiered_stops},
    }
...
def compare_v5_v51(targets, cfg=None):
    ...
    for sym in targets:
        ...
        print(
            f"v5: {n5}笔 expR={exp5:.4f} win={win5:.1%} dd={dd5:.2f}R | "
            f"v5.1: {n51}笔 expR={exp51:.4f} win={win51:.1%} dd={dd51:.2f}R "
            f"(质量:{qf} 周期:{tff}) ΔexpR={de:+.4f} Δdd={dd:+.2f}R",
            flush=True,
        )
    ...
    print(
        f"  品种: {len(valid)} 个 | Δ笔数: {avg_dt:+.1f} | ΔexpR: {avg_de:+.4f} | Δ胜率: {avg_dw:+.1%} | Δdd: {avg_dd:+.2f}R"
    )
