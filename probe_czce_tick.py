...
def main():
    ...
    def live_quote(sym):
        ...
        try:
            ...
            signal.alarm(0)
            print(
                f"  ✅ live OK：last={q.get('last_price')} name={q.get('ins_name')} dt={q.get('datetime')}", flush=True
            )
            api.close()
        except TimeoutError:
            print("  ⏱ 超时", flush=True)
        ...
        try:
            ...
            while api.wait_update():
                if len(tk) > n:
                    n = len(tk)
                if n >= 3000:
                    print("  (达到 3000 ticks，提前停)", flush=True)
                    break
            signal.alarm(0)
            ...
            if n > 0:
                ...
                print(
                    f"     有 direction={('direction' in keys)} bid1={('bid_volume1' in keys)} "
                    f"ask1={('ask_volume1' in keys)} volume={('volume' in keys)}",
                    flush=True,
                )
            else:
