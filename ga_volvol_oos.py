#!/usr/bin/env python3
"""
ga_volvol_oos.py — 6+Vol_vol 7 因子（6+Vol_vol）板块级 GA 优化 + OOS 走步法验证

对比：基准 vs 6因子稳健版 vs 7因子（6+Vol_vol）稳健版
...
# ===== 6+Vol_vol 7 因子列表 =====
SF7 = ["T_trend", "T_mean", "T_seasonal", "F_basis", "F_seasonal", "C", "Vol_vol"]
...
def main():
    ...
    print("=== 6+Vol_vol 7 因子 GA + OOS 验证 ===")
    print(f"因子: {', '.join(SF7)}")
    ...
    for sector in sectors:
        ...
        # 4. 6+Vol_vol 7 因子 GA（训练集）
        print("  6+Vol_vol 7 因子 GA 训练...")
        ...
        sr_w = ga7_result["weights"].get("Vol_vol", 0)
        print(f"    Vol_vol 权重: {sr_w:.4f} ({sr_w * 100:.1f}%)")

        # 5. 6+Vol_vol 7 因子 OOS
        print("  6+Vol_vol 7 因子 OOS 验证...", end=" ", flush=True)
        ...
        print(
            f"  Vol_vol 增量: {sr_delta_oos:+.4f} ({'+' if sr_delta_oos > 0 else ''}{sr_delta_oos / (abs(ga6_oos['expR']) + 1e-8) * 100:.1f}%)"
        )
    ...
    print("=" * 80)
    print(f"{'板块':<8}{'基准':>10}{'6因子':>10}{'7因子':>10}{'6因子提升':>12}{'7因子提升':>12}{'Vol_vol增量':>10}")
    ...
    if args.save:
        out_file = os.path.join(HERE, "logs", "ga_volvol_oos.json")
