...
PER_SYMBOL_RISK = {
    "JM": {"single_leg": 0.18, "strict_stop": True, "note": "焦煤低胜率(27%)：单笔≤18%、强制止损不可放宽"},
    "J": {"single_leg": 0.18, "strict_stop": True, "note": "焦炭低胜率(34%)：单笔≤18%、强制止损不可放宽"},
}
...
def risk_guard(
    ...
    if status == "OK":
        ...
        if equity > 0 and proposed_margin / equity >= single_leg:
            ...
            reasons.append(
                f"单笔保证金占比 {proposed_margin / equity * 100:.0f}% 超 {single_leg * 100:.0f}%"
    ...
    def update(self, rg, equity=None):
        with self._lock:
            ...
            if status == "LOCK":
                ...
                _can_release = (
                    usage < RED_LINE
                    and (now - self.entered_at) > LOCK_RELEASE_SEC
                    and not self.daily_loss_locked
                    and not self.consec_lock
                )
                if _can_release:
...
class KillSwitch:
                ...
                json.dump(
                    {
                        ...
                        "flatten_plan": self.flatten_plan,
    ...
    # ---------- 核心判定 ----------
    def check(self, equity, peak_equity=None, daily_pnl=0.0, consec_losses=0, positions=None):
    ...
    def acknowledge(self):
        ...
        with self._lock:
            if self.halted:
                self.ack = True
                self.history.append({"t": time.strftime("%Y-%m-%d %H:%M:%S"), "event": "ACK"})
                self._save()
            return self.summary()
    ...
    def reset(self, note="人工解除", reset_peak_to=None):
        ...
        with self._lock:
            ...
            if was:
                self.history.append({"t": time.strftime("%Y-%m-%d %H:%M:%S"), "event": "RESET", "note": note})
            self._save()
            if reset_peak_to:
                try:
...
# 全局单例（default 账户，向后兼容）
RISK_FSM = RiskStateMachine()
KILL = KillSwitch()

# ============ 多账户支持（2026-09-07） ============
# 设计原则：
#   · default 账户 → 沿用原全局单例 RISK_FSM / KILL，状态文件不变
#   · 其他账户  → 按需创建实例，状态文件为 killswitch_state_{account_id}.json
#   · 工厂函数 get_fsm / get_kill 负责按 account_id 缓存实例
#   · 线程安全：用 _REGISTRY_LOCK 保护实例字典（实例内部自带 _lock）
...
def _kill_state_file_for(account_id):
    """按账户返回 killswitch 状态文件路径，default 沿用旧文件名。"""
    if account_id == "default":
        return KILL_STATE_FILE
    return os.path.join(_HERE, f"killswitch_{account_id}.json")


def get_fsm(account_id=None):
    """获取指定账户的 RiskStateMachine 实例，不传则用 default。

    default 账户直接返回全局单例 RISK_FSM（确保向后兼容）；
    其他账户按需创建并缓存。"""
...
def get_kill(account_id=None):
    """获取指定账户的 KillSwitch 实例，不传则用 default。

    default 账户直接返回全局单例 KILL（确保向后兼容）；
...
def _kill_halted(account_id=None):
    try:
...
def update_risk_state(
    ...
    """
    fsm = get_fsm(account_id)
    kill = get_kill(account_id)
    if consec_losses is not None:
        with fsm._lock:
            fsm.consec_losses = max(0, int(consec_losses))
    if peak_equity is not None:
        try:
            pe = float(peak_equity)
            with fsm._lock:
                if fsm.peak_equity is None or pe > fsm.peak_equity:
                    fsm.peak_equity = pe
        except Exception:
            pass
...
def get_combined_risk_scale(account_id=None):
    ...
    if _kill_halted(account_id):
        ...
        reasons.append(f"状态机锁定(LOCKED): {fsm.lock_reason}")
    elif rsm_state == RiskStateMachine.WARNING:
        reasons.append("状态机预警(WARNING): 手数按 0.5x 缩放")

    # 2. drawdown_guard 侧（多账户版：通过 get_guard 取对应账户）
    ddg_scale = 1.0
    dd_tier = 0
    dd_pct = 0.0
    if _DUAL_TRACK_INIT:
...
if __name__ == "__main__":
    ...
    pos = [
        {"symbol": "FG", "name": "玻璃", "direction": "多", "lots": 3, "price": 1200},
        {"symbol": "jd", "name": "鸡蛋", "side": "short", "qty": 2, "price": 3400},
    ]
    s2 = update_risk_state(840000, used_margin=100000, daily_pnl=-20000, consec_losses=1, positions=pos)
    ks = s2["killswitch"]
