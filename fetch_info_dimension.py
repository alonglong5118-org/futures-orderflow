    ...
    try:
        ...
        if df is not None and len(df):
            ...
            if col:
                ...
                out.append({"text": f"生猪现货成交均价{'上行' if s>0 else '下行'}（近周）",
                            "score": round(s, 2), "source": "ak.index_hog_spot_price", "ts": _ts(),
                            "sym": "lh"})
        # 供应（能繁/出栏）指标（列: value）
        ...
        if sup is not None and len(sup):
            ...
            if val_col:
                ...
                # 供应↑ 偏空（供过于求）
                out.append({"text": f"生猪供应指标{'增加' if s>0 else '减少'}",
                            "score": round(-s, 2), "source": "ak.futures_hog_supply", "ts": _ts(),
                            "sym": "lh"})
    except Exception as e:
    ...
    try:
        ...
        if df is not None and len(df):
            ...
            if chg_col:
                ...
                out.append({"text": f"地产景气指数{'改善' if s > 0 else '走弱'}（{chg_col}）",
                            "score": round(s * 0.8, 2), "source": "ak.macro_china_real_estate",
                            "ts": _ts(), "sym": None})  # 宏观类→全部
    except Exception as e:
...
def _spot(items):
    ...
    try:
        ...
        for _, r1 in f1.iterrows():
            ...
            if abs(score) < 0.02:
                continue  # 噪声过滤
            out.append({"text": f"{sym} 现货基差率{'收敛(偏多)' if score > 0 else '走阔(偏空)'}",
                        "score": round(score, 2), "source": "ak.futures_spot_price",
                        "ts": _ts(), "sym": sym_key[sym]})
    except Exception as e:
...
def _macro(items):
    ...
    try:
        ...
        if rr is not None and len(rr):
            ...
            if abs(s) > 0.01:
                out.append({"text": f"存款准备金率{'下调(降准·利多)' if s>0 else '上调(利空)'}",
                            "score": round(s * 0.6, 2), "source": "ak.macro_china_reserve_requirement_ratio",
                            "ts": _ts(), "sym": None, "macro": True})
        # PMI
        ...
        if pmi is not None and len(pmi):
            ...
            out.append({"text": f"制造业PMI{'扩张' if s>0 else '收缩'}",
                        "score": round(s * 0.5, 2), "source": "ak.macro_china_pmi",
                        "ts": _ts(), "sym": None, "macro": True})
        # CPI / PPI
        for fn_name, label in [("macro_china_cpi", "CPI"), ("macro_china_ppi", "PPI")]:
            ...
            if df is not None and len(df):
                ...
                out.append({"text": f"{label} 同比{'上行' if s>0 else '回落'}",
                            "score": round(s * 0.4, 2), "source": f"ak.{fn_name}",
                            "ts": _ts(), "sym": None, "macro": True})
    except Exception as e:
                ...
                for v in df[txt_col].astype(str).tolist()[:20]:
                    ...
                    for w in bear:
                        if w in v:
                            sc -= 0.3
                    if sc != 0:
                        out.append({"text": f"快讯：{v[:40]}", "score": round(max(-0.6, min(0.6, sc)), 2),
                                    "source": "ak.news_economic_baidu", "ts": _ts(),
                                    "sym": None, "macro": True})
    except Exception as e:
