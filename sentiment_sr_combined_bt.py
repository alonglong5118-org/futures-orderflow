    ...
    while i < n - 1:
        ...
        try:
            pipe = pipeline(symbol, hist, None, DEFAULT_CONFIG,
                           date=date_str, sentiment_label=sent_band)
        except Exception:
            i += 1
            continue
...
def main():
    ...
    print()
    print("=" * 100)
    print(f"{'模式':<16} {'有效':>4} {'提升':>4} {'下降':>4} {'持平':>4} "
    ...
    print(f"{'基准':<16} {len(base_valid):>4}    0    0 {len(base_valid):>4} "
          f"{base_avg:>+8.4f}   +0.0%  {base_wr:>5.1%}  {base_rr:>5.2f}")

    summary = {"base": {"avg_expR": round(base_avg, 4), "count": len(base_valid),
                        "avg_win_rate": round(base_wr, 3), "avg_rr_ratio": round(base_rr, 3)}}
    ...
    for grp in sorted(groups.keys()):
        ...
        sent_g = [next((r["expR"] for r in all_results["sentiment"] if r["symbol"] == s and "expR" in r), 0) for s in syms]
        sr_g = [next((r["expR"] for r in all_results["sr_widen"] if r["symbol"] == s and "expR" in r), 0) for s in syms]
        comb_g = [next((r["expR"] for r in all_results["combined"] if r["symbol"] == s and "expR" in r), 0) for s in syms]
