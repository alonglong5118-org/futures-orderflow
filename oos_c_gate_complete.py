...
def split_pop(pop, win_key, loss_key):
    return (
        [r for r in pop if r.get(win_key)],
...
def main():
    ...
    for s in missing:
        ...
        if rec.get("skip"):
            ...
            print(
                f"    [有效] Δot={rec.get('delta_ot_vs_base')} ot拦={rec.get('ot_gate_skipped')} "
    ...
    valid = [r for r in complete if r.get("base_oos", {}).get("n", 0) > 0 and r.get("ot_oos", {}).get("n", 0) > 0]
    ...
    summary = {
        ...
        "newly_added": [
            {
                ...
                "ot_skipped": r.get("ot_gate_skipped"),
            }
            for r in new_recs
        ],
        ...
        "delta_mean_ss_vs_base": round(mean_ss - mean_base, 4)
        if (mean_ss is not None and mean_base is not None)
        else None,
        "delta_mean_ot_vs_base": round(mean_ot - mean_base, 4)
        if (mean_ot is not None and mean_base is not None)
        else None,
        "n_oos_win_ss": len(ss_win),
        "n_oos_loss_ss": len(ss_loss),
        "n_oos_flat_ss": len(ss_flat),
        "n_oos_win_ot": len(ot_win),
        "n_oos_loss_ot": len(ot_loss),
        "n_oos_flat_ot": len(ot_flat),
        "significance_same_sign": sig_ss,
        "significance_oppose": sig_ot,
        "note": (
            "完整活跃池 %d 只全覆盖；其中 OOS 已验证（基线有成交且门可用）= %d 只（即 prior 41，门可用总体）；"
    ...
    print(
        f"  oppose 二项p    : {sig_ot.get('binom_p_two_sided')}  "
        f"(增益{sig_ot.get('wins')}/有损{sig_ot.get('losses')}/持平{sig_ot.get('flats')})"
    )
    for r in new_recs:
        print(
            f"  [新] {r['symbol']}: status={r.get('status')} Δot={r.get('delta_ot_vs_base')} "
