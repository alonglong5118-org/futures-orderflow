...
def main():
    ...
    print()
    print("=" * 100)
    print(f"{'参数':>8} {'有效':>4} {'提升':>4} {'下降':>4} {'持平':>4} "
    ...
    print(f"{'基准':>8} {len(base_results):>4}    0    0 {len(base_results):>4} "
          f"{base_avg:>+8.4f}   +0.0%  {base_wr:>5.1%}  {base_rr:>5.2f}        —          —")

    summary = {"base": {"avg_expR": round(base_avg, 4), "count": len(base_results),
    ...
    for mult in multipliers:
        ...
        label = f"{mult}R"
        print(f"{label:>8} {len(valid):>4} {improved:>4} {worsened:>4} {unchanged:>4} "
    ...
    for grp, syms in sorted(groups.items()):
        ...
        for mult in multipliers:
            ...
            if delta > best_delta:
                best_delta = delta
                best_mult = mult
                best_data = {"delta": delta, "count": len(valid_syms),
                             "base_expR": base_grp_expR, "sr_expR": sr_grp_expR}
        ...
        if best_data:
            ...
            print(f"{grp:<8} {best_data['count']:>4} {best_mult:>4}R  "
