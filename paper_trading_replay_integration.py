"""
paper_trading_replay_integration.py · 实盘跟踪引擎（第二账户）
=================================================================
第二个 PaperTradingEngine 实例，用于跟踪真实实盘持仓。
- 不跟随策略信号自动开仓（只手动管理）
- 启用 TP/SL 检查 + 移动止损（盈亏和风控实时计算）
- 独立状态文件：paper_trading_replay_state.json
- 独立 API 路由：/api/paper-replay
- 独立仪表盘：/paper_replay_dashboard/

用法（在 four_dim_live_runner.py 中）：
  import paper_trading_replay_integration as ptri

  # 初始化（main 函数开头，pti.init 之后）
  ptri.init(feed, contract_specs=_TCFG.get("contract_specs", {}))

  # 每轮循环调用（只检查持仓 TP/SL，不跟信号）
  ptri.tick(state)

  # /api/state 中注入
  state["paper_replay"] = ptri.get_state()

  # /api/paper-replay 端点处理
  if path == "/api/paper-replay":
      ptri.handle_api(self)
"""
...
STATE_FILE = os.path.join(HERE, "paper_trading_replay_state.json")
...
def init(price_feed=None, contract_specs: dict | None = None, config: dict | None = None):
    ...
    默认配置：
    - enabled: False（不跟随策略信号自动开仓，只手动管理）
    ...
    # 状态已通过构造函数的 state_file 参数加载

    _initialized = True
    print(f"[PaperReplay] 实盘跟踪引擎已初始化（启用自动开仓: {_engine.config['enabled']}）")
    return _engine
...
def tick(state: dict | None = None) -> dict:
    ...
    Args:
        state: live runner 的 state 字典，用于注入 paper_replay 状态
    ...
    try:
        ...
        print(f"[PaperReplay] 检查持仓异常: {e}")

    # 注入到 state
    if state is not None:
        try:
            state["paper_replay"] = get_state()
        except Exception:
            pass
...
def get_state() -> dict:
    """获取实盘跟踪状态（供前端展示）。"""
    if not _engine:
        return {"enabled": False, "error": "未初始化", "mode": "replay"}
    try:
        s = _engine.get_state()
        s["mode"] = "replay"  # 标记为实盘跟踪模式
        return s
    except Exception as e:
        return {"enabled": False, "error": str(e), "mode": "replay"}


def handle_api(handler) -> None:
    """处理 /api/paper-replay API 请求。
