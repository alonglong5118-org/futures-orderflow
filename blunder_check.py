            ...
            if le[1] != direc:
                blunders.append(
                    _b(
                        ...
                        sym,
                        t.get("time", ""),
                        f"{le[1]}→{direc} 在 {FLIP_WINDOW_MIN}分钟内翻转",
                        "反手=认错，先平再看，不要对着干",
                    )
                )
        ...
        if prev_pnl is not None and prev_pnl < 0 and lots > (last_entry.get(sym, (0, "", 0))[2] or 0):
            blunders.append(
                _b(
                    ...
                    sym,
                    t.get("time", ""),
                    f"上一笔亏损后本笔加至 {lots}手（martingale）",
                    "亏了就缩手，绝不加码摊平",
                )
            )
            ...
            if R <= LOSS_R_THRESHOLD:
                blunders.append(
                    _b(
                        ...
                        sym,
                        t.get("exit_time", ""),
                        f"该笔 R={R:.2f}（亏 {t.get('pnl')}元）",
                        "复盘：是信号错还是执行错？别让一笔拖垮一周",
                    )
                )
...
def _b(type_, sev, sym, time_, detail, suggestion):
    return {"type": type_, "sev": sev, "symbol": sym, "time": time_, "detail": detail, "suggestion": suggestion}
