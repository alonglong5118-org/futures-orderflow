# -*- coding: utf-8 -*-
"""
ga_tpsl_optimizer_v4_lite.py — 3 参数精简 GA 优化器（Phase 3 Lite）
==================================================================
相对 Phase 3 v3 的精简：
    - 只优化 3 个最关键参数（入场阈值 + 止损 + 止盈）
    - 其余 6 个参数固定为基线值
    - 大幅减少过拟合空间，提升 OOS 泛化能力
...
用法:
    python3 ga_tpsl_optimizer_v4_lite.py --symbol rb --shrink --l1 --fast --full
"""
...
def run_ga_nsga2(symbol, df_is, train_bars, valid_bars, step_bars, pop_size, gen_count,
                 use_shrink=False, use_l1=False, n_jobs=DEFAULT_N_JOBS,
                 early_stop_patience=EARLY_STOP_PATIENCE):
    """运行 NSGA-II 优化，带移民、早停、并行评估。"""
...
<head>
<meta charset="UTF-8">
<title>GA 参数优化报告 - {symbol} (Phase 3 Lite)</title>
<style>
...
<div class="container">
    ...
    <div class="subtitle">
        Phase 3 Lite · 3 参数联合优化（入场阈值 + 止损 + 止盈） ·
    ...
    <div class="footer">
        Generated by ga_tpsl_optimizer_v4_lite.py · Phase 3 Lite · 3 参数精简优化
    </div>
</div>
</body>
</html>"""

    report_path = os.path.join(output_dir, f"{symbol}-phase3-lite-report.html")
...
def main():
    parser = argparse.ArgumentParser(description="3 参数精简 GA 优化器（Phase 3 Lite）")
    ...
    # 输出目录
    output_dir = args.output or os.path.join(HERE, f"ga_v4_{symbol}_result")
    os.makedirs(output_dir, exist_ok=True)

    print(f"🚀 Phase 3 Lite GA 优化启动: {symbol}")
    ...
    print(f"\n🧬 开始 GA 优化...")
    result = run_ga_nsga2(
        symbol, df_is, train_bars, valid_bars, step_bars,
        pop_size, gen_count,
    ...
    json_path = os.path.join(output_dir, f"{symbol}_phase3_lite_result.json")
