...
def check_3_mixed_json_types(code, _path):
    ...
    for m in re.finditer(r"json\.loads?\s*\(", code):
        ...
        if ".get(" in body_region and "isinstance" not in body_region:
            ...
            issues.append((lineno, f"json.load 变量 `{var}` 迭代后 .get() — 数据文件混入 list key 会炸"))
    return issues
...
def check_6_tdz_global(code, _path):
    ...
    for block in SCRIPT_BLOCK_RE.findall(code):
        ...
        def in_fn(li, _ranges=fn_ranges):
            return any(s <= li <= e for s, e in _ranges)
        ...
        for i, line in enumerate(slines):
            ...
            for m in re.finditer(r"let\s+([a-zA-Z_]\w*)\s*[=;]", line):
                var = m.group(1)
                if len(var) <= 2 or var.lower() in exclude:
                    continue
...
def check_7_tab_refresh(code, _path):
    ...
    for m in re.finditer(r"if\(name==='(\w+)'\)\{(.*?)\}", code, re.DOTALL):
        ...
        if len(calls) == 0 and tab not in EXEMPT:
            issues.append((code[: m.start()].count("\n") + 1, f"switchTab('{tab}') 无 refresh — 子面板可能永远不加载"))
    return issues
...
def run_lint(filepath):
    ...
    for name, fn in BLOCKING:
        for lineno, msg in fn(code, filepath):
            r["blocking"].append((name, lineno, msg))
    for name, fn in WARNING:
        for lineno, msg in fn(code, filepath):
            r["warning"].append((name, lineno, msg))
    return r
...
def report(r):
    ...
    if r["blocking"]:
        print(f"  🔴 BLOCKING ({bl}):")
        for n, l, m in r["blocking"]:
            print(f"    L{l:>5} [{n}] {m}")
    if r["warning"]:
        print(f"  🟡 WARN ({wl}):")
        for n, l, m in r["warning"][:10]:
            print(f"    L{l:>5} [{n}] {m}")
        if wl > 10:
            print(f"    ... +{wl - 10} more")
    print(f"  → 🔴{bl} / 🟡{wl}")
