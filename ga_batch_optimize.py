...
def optimize_one(symbol, pop_size, n_gen, tail):
    ...
    try:
        t0 = time.time()
        result = gfm.optimize_weights(symbol, pop_size=pop_size, n_gen=n_gen, verbose=False, tail=tail)
...
def main():
    ...
    for idx, sym in enumerate(todo):
        ...
        if err:
            ...
            print(f"  ✓ 完成 ({elapsed:.0f}s): expR={expR:.4f}  win={wr * 100:.1f}%  robust={rob:.3f}", flush=True)
        ...
        if done_count > 0:
            ...
            print(
                f"  进度: {done_count}/{len(todo)} ({done_count / len(todo) * 100:.1f}%) "
...
def print_summary(cache):
    ...
    for sym, data in sorted_syms:
        ...
        print(
            f"{sym:<6} {expR:>8.4f} {wr * 100:>7.1f}% {rob:>8.3f} "
