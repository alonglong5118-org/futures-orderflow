...
def main():
    ...
    for sym in need:
        ...
        if not ok:
            res[sym] = dict(ok=False, err=str(payload))
            print(f"    FAIL {payload}", flush=True)
            continue
        fresh = payload
        ex = load_existing(sym)
        if ex is None:
            res[sym] = dict(ok=False, err="无现有合并文件")
            print("    WARN 跳过", flush=True)
            continue
        ex_max = ex["date"].max()
        tail = fresh[fresh["date"] > ex_max]
        if len(tail) == 0:
            res[sym] = dict(ok=True, appended=0, last=str(ex["date"].iloc[-1]), note="天勤尾部未超出现有")
            print("    无新增(尾部未超出)", flush=True)
            continue
        merged = pd.concat([ex, tail[COLS]]).drop_duplicates(subset=["date"]).sort_values("date")[COLS]
        for p in (os.path.join(DATA_5M, f"_{sym}0_min5.csv"), os.path.join(BACKTEST_DIR, f"_{sym}0_min5.csv")):
            merged.to_csv(p, index=False)
        res[sym] = dict(
            ok=True,
            appended=int(len(tail)),
