...
def decide_and_eval(sym):
    ...
    # ── 决策（仅 IS）：是否满足启用护栏 ──
    is_edge = (
        (st_com_is["expR"] - st_thr_is["expR"])
        if (st_com_is["expR"] is not None and st_thr_is["expR"] is not None)
        else None
    )
    enable = False
    block = []
    ...
    delta_sel_vs_base = round(sel_expR - base_expR, 4) if sel_expR is not None and base_expR is not None else None
    ...
    return {
        ...
        "oos_edge_transfer": (
            round(st_com_oos["expR"] - st_thr_oos["expR"], 4)
            if (st_com_oos["expR"] is not None and st_thr_oos["expR"] is not None)
            else None
        ),
        # OOS 验证判读
...
def main():
    ...
    for sym in targets:
        ...
        print(
            f"{sym:5}{rec['group']:5} {ie:>+8.3f} {rec['decision']:>12} "
            f"{sev:>8}{bev:>9}{nev:>10}{dvs:>11}  ({time.time() - t0:.1f}s)",
            flush=True,
        )
    ...
    blocked_naive_better = [
        r
        for r in blocked
        if (
            r["naive_oos"]["expR"] is not None
    ...
    summary = {
        ...
        "n_valid": len(valid),
        "n_enabled": n_enabled,
        "n_decided_off": n_decided_off,
        "mean_oos_expR_selective": mean_sel,
        "mean_oos_expR_baseline": mean_base,
        "mean_oos_expR_naive_combined": mean_naive,
        "delta_mean_sel_vs_base": (
            round(mean_sel - mean_base, 4) if mean_sel is not None and mean_base is not None else None
        ),
        "delta_mean_sel_vs_naive": (
            round(mean_sel - mean_naive, 4) if mean_sel is not None and mean_naive is not None else None
        ),
        "sum_total_R_selective": sum_sel,
        "sum_total_R_baseline": sum_base,
        "sum_total_R_naive": sum_naive,
        "n_oos_win": len(wins),
        "n_oos_loss": len(losses),
        "n_oos_flat": len(flats),
        "n_enabled_true_wins": len(true_wins),
        "n_enabled_false_pos": len(false_pos),
        "n_blocked": len(blocked),
        "n_blocked_naive_would_help": len(blocked_naive_better),
        "config": {
            "SPLIT": SPLIT,
            "MIN_T": MIN_T,
            "MARGIN": MARGIN,
    ...
    print(
        f"  逐品种 OOS 期望R均值：选择性 {mean_sel:+.4f} ｜ 基线(永远thr) {mean_base:+.4f} ｜ 朴素(永远comb) {mean_naive:+.4f}"
    )
    if summary["delta_mean_sel_vs_base"] is not None:
        print(
            f"  Δ选择性−基线 = {summary['delta_mean_sel_vs_base']:+.4f} ｜ Δ选择性−朴素 = {summary['delta_mean_sel_vs_naive']:+.4f}"
        )
    print(f"  OOS 相对基线：增益 {len(wins)} / 有损 {len(losses)} / 持平 {len(flats)}")
    print(f"  启用品种中 真赢 {len(true_wins)} / 假阳(启用却OOS不及基线) {len(false_pos)}")
    print(
        f"  护栏拦下 {len(blocked)} 个；其中若朴素 combined 本更优的有 {len(blocked_naive_better)} 个（潜在误杀）",
        flush=True,
    )
