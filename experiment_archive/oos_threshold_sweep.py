...
from oos_c_gate import _load_kline_cache, run_backtest, split_is_oos, subset_stats
...
targets = [
    s
    for s in fd.SYMBOLS
    if s.upper() in cache and s not in ("SA01",) and s not in set(getattr(fd, "DISABLED_SYMBOLS", []) or [])
]
targets.sort()
...
def oos_mean(sym, thr):
    ...
    bs = subset_stats(base_oos)
    os_ = subset_stats(ot_oos)
    if os_["expR"] is None or bs["expR"] is None:
        return None
    return {
        "base": bs["expR"],
        "ot": os_["expR"],
        "delta": round(os_["expR"] - bs["expR"], 4),
        "ot_n": os_["n"],
        "base_n": bs["n"],
    }
...
for thr in (30, 40, 50):
    ...
    for s in targets:
        ...
        d = r["delta"]
        deltas.append(d)
        base_means.append(r["base"])
        ot_means.append(r["ot"])
        if d > 0:
            wins += 1
        elif d < 0:
            losses += 1
...
print(
    f"{'thr':>4} {'n':>4} {'Δ均值':>8} {'Δ中位':>8} {'基线OOS':>9} {'opposeOOS':>10} {'增益':>4} {'有损':>4} {'持平':>4} {'占比':>6} {'p':>8}"
)
for r in rows:
    print(
        f"{r['thr']:>4} {r['n']:>4} {r['mean_delta']:>8} {r['median_delta']:>8} "
