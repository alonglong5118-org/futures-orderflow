...
def generate_report(output_path=None):
    ...
    for t in trades:
        ...
        cum_R += R
        equity = cum_R
        equity_curve.append({"idx": len(equity_curve), "R": round(R, 4),
                            "equity": round(equity, 2), "symbol": t.get("symbol", ""),
                            "time": t.get("time", "")})
        if equity > peak:
            peak = equity
        dd = equity - peak
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=("权益曲线", "水下曲线 (Drawdown)",
                        "逐笔 R 散点", "盈亏分布直方图",
                        "月度收益热力图", "统计摘要"),
    ...
    fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines", name="累计R",
                            line=dict(color=DARK_THEME["gold"], width=2)), row=1, col=1)

    # 2. 水下曲线
    dd_ys = [d["dd"] for d in dd_curve]
    fig.add_trace(go.Scatter(x=xs, y=dd_ys, mode="lines", name="回撤",
                            fill="tozeroy",
                            line=dict(color=DARK_THEME["red"], width=1)), row=2, col=1)

    # 3. R 散点
    colors = [DARK_THEME["green"] if e["R"] > 0 else DARK_THEME["red"] for e in equity_curve]
    fig.add_trace(go.Scatter(x=xs, y=[e["R"] for e in equity_curve],
                            mode="markers", name="单笔R",
                            marker=dict(color=colors, size=5)), row=3, col=1)

    # 4. 盈亏直方图
    r_vals = [e["R"] for e in equity_curve]
    fig.add_trace(go.Histogram(x=r_vals, name="R分布",
    ...
    profit_factor = sum(float(t.get("R", 0)) for t in wins) / abs(sum(float(t.get("R", 0)) for t in losses)) if losses else 0

    fig.add_trace(go.Table(
        header=dict(values=["指标", "值"],
                    fill=dict(color=DARK_THEME["grid"]),
                    font=dict(color=DARK_THEME["text"])),
        cells=dict(values=[
            ["交易笔数", "胜率", "平均盈利R", "平均亏损R", "总R", "最大回撤R", "盈亏比"],
            [f"{n_trades}", f"{win_rate:.1f}%", f"{avg_win:.2f}", f"{avg_loss:.2f}",
             f"{total_R:.2f}", f"{max_dd:.2f}", f"{profit_factor:.2f}"]
        ],
...
def data():
    ...
    for t in trades:
        ...
        if cum > peak:
            peak = cum
        equity.append({"R": round(R, 2), "cum_R": round(cum, 2),
                       "dd": round(cum - peak, 2), "symbol": t.get("symbol", "")})
